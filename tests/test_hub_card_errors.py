...
def _make_card(health: dict, last_result=None):
    """Instantiate HubCard with mocked health storage."""
...
def test_set_error_pip_shows_install_button(qapp):
    card = _make_card(_FRESH_HEALTH)
    card.set_error(
...
def test_set_error_raw_pip_message_shows_button(qapp):
...
def test_set_error_generic_hides_install_button(qapp):
    card = _make_card(_FRESH_HEALTH)
    card.set_error("Connection timeout — device not responding")
    assert _hid(card._btn_install)


def test_set_error_auth_failure_hides_install_button(qapp):
    card = _make_card(_FRESH_HEALTH)
    card.set_error("Authentication failed — check your password.")
    assert _hid(card._btn_install)


def test_set_error_dot_turns_red(qapp):
...
def test_set_error_updates_metrics_label(qapp):
    card = _make_card(_FRESH_HEALTH)
    card.set_error("Authentication failed — check your password.")
...
def test_disabled_card_shows_reenable_button(qapp):
    card = _make_card(_DISABLED_HEALTH)
    assert _vis(card._btn_reenable)


def test_disabled_card_refresh_is_disabled(qapp):
    card = _make_card(_DISABLED_HEALTH)
    assert not card._btn_refresh.isEnabled()


def test_fresh_card_reenable_hidden(qapp):
    card = _make_card(_FRESH_HEALTH)
    assert _hid(card._btn_reenable)


def test_fresh_card_refresh_enabled(qapp):
    card = _make_card(_FRESH_HEALTH)
...
def test_plugin_device_page_banner_pip_shows_button(qapp):
    ...
    assert _vis(page._banner_install_btn)
    assert "tplinkrouterc6u" in page._banner_install_btn.text()


def test_plugin_device_page_banner_generic_hides_button(qapp):
    from ui.pages.plugin_device_page import PluginDevicePage
    ...
    page._show_banner("Cannot reach the device — is it online?", AMBER)
    assert _hid(page._banner_install_btn)


def test_plugin_device_page_banner_visible_on_call(qapp):
    from ui.pages.plugin_device_page import PluginDevicePage
    from ui.styles import RED
    page = PluginDevicePage(
        plugin_path="/fake/plugin.py",
    ...
    assert _hid(page._banner_frame)
    page._show_banner("Some error", RED)
    assert _vis(page._banner_frame)
