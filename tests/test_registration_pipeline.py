...
class _PageStub:
    ...
    def __init__(self):
        ...
        self._show_credential_dialog = MagicMock(return_value=(True, "192.168.1.1"))
...
def _qs_store() -> dict:
    """Return a simple in-memory QSettings-like store and a patch factory."""
    store: dict = {}
    ...
    return store, _FakeQS


# ── _show_credential_dialog return type ───────────────────────────────────────

def test_credential_dialog_return_annotation_is_tuple():
    """_show_credential_dialog must declare a tuple return annotation."""
    from ui.pages.hardware_integration_page import HardwareIntegrationPage
    sig = inspect.signature(HardwareIntegrationPage._show_credential_dialog)
    ann = sig.return_annotation
    assert ann is not inspect.Parameter.empty, (
        "_show_credential_dialog must have a return annotation"
    )
    # The annotation should be tuple[bool, str] or equivalent
    ann_str = str(ann)
...
def test_credential_dialog_signature_has_two_elements():
    ...
    ann = sig.return_annotation
    ann_str = str(ann)
    # Accept both "tuple[bool, str]" and "Tuple[bool, str]" forms
...
def test_register_plugin_writes_instance_registry(tmp_path):
    ...
    with patch("ui.pages.hardware_integration_page.QSettings", FakeQS), \
...
def test_browse_and_bundled_produce_identical_instance_entries(tmp_path):
    ...
    # Same keys present regardless of source
...
def test_cancel_credential_dialog_does_not_write_registry(tmp_path):
    ...
    page = _PageStub()
    page._show_credential_dialog = MagicMock(return_value=(False, ""))  # cancelled
...
def test_no_credential_label_skips_dialog(tmp_path):
    ...
    page = _PageStub()
    dialog_mock = MagicMock(return_value=(True, ""))
    page._show_credential_dialog = dialog_mock
...
def test_register_plugin_does_not_emit_for_duplicate(tmp_path):
    ...
    with patch("ui.pages.hardware_integration_page.QSettings", FakeQS), \
         patch("ui.pages.hardware_integration_page._load_instances",
...
def test_register_plugin_uses_confirmed_ip_from_dialog(tmp_path):
    ...
    page = _PageStub()
    # User changed the IP in the dialog to something different from meta default
    page._show_credential_dialog = MagicMock(return_value=(True, "10.0.0.254"))
    ...
    assert len(saved_instances) == 1
    # The instance must record the IP the user actually entered
