...
def test_comment_includes_evidence_when_available(w):
    ...
    # Looked up, never `"host" in <url-ish>`: that shape is what CodeQL
    # py/incomplete-url-substring-sanitization fingerprints, false positive or not.
    vt = hosts.get("www.virustotal.com") or []
    assert vt and vt[0].path == "/gui/url/abc"
    installer = hosts.get("example.invalid") or []
    assert installer and installer[0].path.endswith("NetSentinel-Setup-2.3.0.exe")
