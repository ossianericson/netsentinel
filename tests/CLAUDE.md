...
<!-- Source: local .claude/rules/tests.instructions.md -->
# Test Conventions
...
always patch at the **definition site**, not the re-export site.
...
Files currently re-exporting from `ui/widgets/`:

| Re-export file | Definition file | Symbols |
|---|---|---|
| `ui/pages/hardware_integration_page.py` | `ui/widgets/hub_card.py` | all hub_card symbols |
| `ui/pages/hardware_integration_page.py` | `ui/widgets/hub_helpers.py` | helper functions (post S15-2 split) |

Rule: `grep -r "from ui.widgets" ui/pages/` reveals all re-export relationships.
When in doubt, check where the function body actually lives before patching.
