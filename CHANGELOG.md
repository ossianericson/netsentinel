...
**Added**
- Session records (`modules/session_record.py`) — a record is written `clean_exit=false` at startup and flipped only by an explicit shutdown, so *not* being marked clean is the signal. Carries the last page reached, which is the only thing that can say where the user was when the OS took the process. This separates "the user quit" from "Windows killed us for memory" from a native FailFast — the classes Partner Center labels *Unknown*, and the ones that produce no traceback and no `faulthandler` entry
- Environment fingerprint (`modules/environment_fingerprint.py`) — measures the ANSI/OEM codepage pair, `GetUserDefaultLocaleName()`, the preferred and filesystem encodings, timezone and UTC offset, and packaging/admin/Npcap state. The v2.2.8 triage had to *infer* each affected machine's codepage pair from its market code; this measures it. Deliberately carries no IP, MAC, SSID, hostname, username or path, enforced by a closed allowlist test — it writes files and sends nothing, so PRIVACY.md's zero-telemetry promise is untouched
- Diagnostic report export (`modules/diagnostic_report.py`), offered from the existing feedback dialog — the environment fingerprint, every session that never shut down cleanly, bounded tails of all four log sinks, and the install channel, all routed through the redactor
- "NetSentinel closed unexpectedly last time" strip on the Home page (`ui/widgets/unclean_exit_strip.py`), with a Send-report action. Dismissal is keyed on the dead session's own start time, so dismissing one crash cannot silence the next
- Log rotation (`modules/log_rotation.py`) in all three entry points, ordered before anything opens a log handle. `netsentinel_crash.log` is explicitly exempt — the chaos harness detects native SEH faults by baselining that file's byte size, so a shrink would read as "nothing new"
- `tools/store_health.py` — pulls Partner Center failure data and diffs it release-over-release, grouped by market × package version × event type. Takes either the analytics API or a Failures CSV, so the analysis works with no Azure app registration
- A monthly winget fork keepalive workflow that resets the fork and fails if the submission token is within 45 days of expiry

**Changed**
- The fatal-error dialog now carries Save report and Copy details buttons, with the traceback behind Qt's Show Details pane instead of in the dialog's main text (RULE-A2)
- The console codec and the crash net are now shared modules (`modules/console_codec.py`, `modules/crash_net.py`) installed by every entry point, rather than logic living at `app.py` module scope
- SMB enumeration runs `net user` only when a domain is supplied. The command has no `\\computer` form — it always enumerates the machine it runs on — so without a domain it described the scanner rather than the target, and reporting the scanner's own accounts as the target's is a worse answer than none

**Fixed**
- `cli.py` and `svc.py` never installed the OEM console-decoding patch, so both shipped binaries carried the exact RULE-WIN21 crash class v2.2.8 fixed for the GUI: `text=True` decodes with the ANSI codepage under `errors='strict'` while console tools emit OEM. `svc.py` runs unattended as a Windows service, where an unhandled `UnicodeDecodeError` has no user present to see it — the service simply stops logging
- `cli.py` and `svc.py` installed no crash net at all — no `faulthandler`, no `excepthook` — so an unhandled exception or a native fault in either binary left no record anywhere
- Nothing anywhere installed `threading.excepthook`, so an exception escaping a plain `threading.Thread` was silently lost. `sys.excepthook` covers the main thread and, via PyQt's routing, escapes from `QThread.run()` — but the SSDP and mDNS discovery threads are plain threads
- `python cli.py --help` raised `UnicodeEncodeError` on stock en-US Windows, not only a localized one: the help text contains `U+2192` and cp1252 cannot encode it. Every non-ASCII hostname, SSID or vendor name the CLI prints was the same crash waiting for a different machine
- Rogue IPv6 router detection matched the English word `Router` in `netsh` output, so the check passed unconditionally on every non-English Windows — a security check that could only ever return "clean"
- WiFi `netsh wlan` channel parsing mis-assigned channels from the second radio onward. The v2.2.8 "structural" fix matched a bare integer on its own line, but Windows 11 emits `QoS MSCS Supported : 0` and `QoS Map Supported : 0` inside each BSSID section, which share that shape. Measured on real output: **7 of 10 BSSIDs read channel 0** before, 0 of 10 after. Channels and signal are now paired by sectioning the block per BSSID instead of zipping two `findall()` lists by index
- Ping and traceroute RTT parsing hard-required the ASCII literal `ms`, which Cyrillic locales translate (`время=44мс`, `<1 мс`), so `icmp_ping()` returned −1.0 — "unreachable" — for every host that answered, and all three traceroute parsers produced no hops at all
- `_parse_traceroute` never reported a Windows RTT, on en-US, always: the unit group was non-capturing and the consumer read a latency only when `system != "Windows"`, so every hop kept its −1.0 default and Service Diagnostics rendered a full column of asterisks
- The POSIX branch of the same function returned `ip=None` for every hop in the C locale. Its pattern was written in the Windows field order (`rtt … ms … ip`), but real `traceroute` prints the address first, so index arithmetic over `m.lastindex` handed back the wrong group
- The `openssh` fallback in credentialed scanning decoded the remote host's output with the local OEM console codepage. The bytes come from the *scanned host*, not the user's locale, so a modern Linux remote's UTF-8 `/etc/passwd` was corrupted on en-US too — `José García` arrived as `Jos├⌐ Garc├¡a` in the Login Test report, silently, because `errors="replace"` never raises. `paramiko`, the same function's other backend, already decoded UTF-8; it is an optional dependency and is not bundled, so the broken fallback was the shipped path
- `sc query type= service state= running` exits 87 with `ERROR: Invalid state= field` — `state=` takes `active`, `inactive` or `all`. The Windows credentialed scan has therefore reported **zero services on every Windows and every locale** since the command was written. Running services are now identified by the untranslated numeric state code, which also correctly excludes the `START_PENDING` and `PAUSED` services `state= active` returns
- SMB `net user` enumeration passed an empty string as a positional argument whenever no domain was given, which Windows reads as a username: exit 1, usage banner, no users at all, on every locale. The account list is now parsed structurally rather than by an English four-word denylist — measured on real output, **8 "accounts" for 5 real ones** before, including the host name out of the header and two words out of the status line, and exactly 5 after. Non-Latin account names, which the `[A-Za-z]`-anchored pattern could not represent at all, now survive
- NetBIOS names were decoded with `ascii`/`errors="ignore"`, which deletes the non-ASCII bytes and returns whatever ASCII survived — so a host registered as `PC-Петров` arrived as `PC-`, a plausible truncated hostname rather than a visible miss
- PDF export built its `file:` URL by string interpolation, so a `#` anywhere in the temporary path — legal in a Windows account name, and `%TEMP%` sits under the profile — truncated the URL at the fragment. Headless Edge then exited 0 and printed **its own "file not found" page as the PDF**, so the export reported success and the user opened a report containing a browser error
- The plugin worker's JSON contract depended on the machine's ANSI codepage: a Python child encodes a piped `stdout` with ANSI while the parent decoded OEM, and the two differ on every Windows install. Both ends are now pinned to UTF-8 — naming the parent's codec alone does not fix it
- winget submission has failed on four releases with "could not be synced". `wingetcreate`'s pre-submit sync is a merge, and the fork's master carries directory-deletion commits for eleven versions that still exist upstream, so a merge preserves them forever and the fork re-diverges within hours of any sync. Submission now hard-resets the fork's master to upstream HEAD, which is the only operation that drops those commits, and the retry loop classifies sync failures instead of burning a rate-limit backoff that could never fix them

**Security**
- Diagnostic reports are redacted through a single extended sanitizer (`modules/report_sanitizer.py`), which now also removes IPv6 addresses (parsed rather than pattern-matched, so clock timestamps survive), SSID values behind their label, and the Windows account name out of `%LOCALAPPDATA%` paths while keeping the path shape that makes a report diagnosable
