...
def _find_and_click_button(win, candidates, log):
    ...
    one found visible+enabled. Returns None if none of the candidates are
    present (e.g. an admin-required banner is covering the button).

    Walks descendants() and matches on window_text(), rather than
    win.child_window(title=..., control_type="Button"): self._win is a
    pywinauto *UIAWrapper* (obtained via Desktop(backend="uia").windows() in
    MonkeyTester._connect), which has no child_window() method. The old call
    raised AttributeError on every candidate, which the bare except swallowed,
    so every scan click was a silent no-op — the log showed 'button_not_found'
    on all 22 iterations with an empty by_control_type. This mirrors the
    descendants() walk already used by _fill_edit_field above."""
    try:
        descendants = win.descendants()
    except Exception:
        descendants = []
    # First wins on duplicate titles; skip hidden/disabled controls so a
    # candidate present but greyed-out (admin banner) doesn't count as found.
    buttons: dict = {}
    for ctrl in descendants:
        try:
            if _mt._safe_type(ctrl) != "Button":
                continue
            if not ctrl.is_visible() or not ctrl.is_enabled():
                continue
            title = _mt._safe_name(ctrl)
            if title and title not in buttons:
                buttons[title] = ctrl
        except Exception:
            continue
    for label in candidates:                 # honour candidate priority order
        ctrl = buttons.get(label)
        if ctrl is None:
            continue
        try:
            ctrl.click_input()
