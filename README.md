...
[![Tests](https://img.shields.io/badge/tests-6101%2B-brightgreen?style=flat-square)](tests/)
...
**6,101+ tests &nbsp;·&nbsp; 9-hour chaos-tested &nbsp;·&nbsp; 100% local &nbsp;·&nbsp; MIT License**
...
**6,101 automated tests** across 463 test files — detection logic, metric storage, version consistency, UI wiring, encoding hygiene, and CodeQL-prevention gates. All tests are offline; no real network traffic or live devices required.
...
- Fixed the minimize button (and other header buttons) becoming unresponsive after restoring the window from the system tray
- Fixed four Security Audit pages (Cloud Metadata Probe, Private Endpoint Check, Recon Plugins, Device Risk Score) that never updated their scan status, so a real finding could sit unreported
- Fixed the device details popover jumping to the wrong monitor when opened on a secondary display
- Fixed several low-contrast colours in the Arctic Clean theme (speed gauge, overview tile hover, topology legend, popover badge)
- Fixed the test suite silently overwriting your saved theme setting back to Arctic Clean after a test run
