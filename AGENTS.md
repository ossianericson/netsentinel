...
| `"Monitoring"` | Monitor | Passive background monitors and live data streams |
| `"Diagnostics"` | Discover, Reports | Network inventory tools and health/diagnostic reports |
| `"Security"` | Security Audit | Admin/audit-flagged active probes (`admin_required=True` or `audit_item=True`) |
| `"Advanced"` | Analysis, Automation | Npcap-required tools, deep-packet analysis, automation/integration |
...
**Key rules:**
- Pages in **Security Audit** nav → `"Security"` group. Pages in **Analysis** nav (Npcap tools) → `"Advanced"` group. Do not put Npcap-gated Analysis tools in `"Security"` even if they detect threats — the nav placement is the canonical authority.
- Pages whose feature-guide entry points to a **Discover-section page** (Devices, DHCP Leases, Network Map, WiFi Networks) → `"Diagnostics"`, consistent with how Network Map, WiFi Networks, and WiFi Heatmap are already classified.
- A feature that already has an entry in `"Start here"` should not have a second entry in `"Security"` — use `"Advanced"` or omit the duplicate.
