...
After GREEN, run the commit gate linters **before** making any structural changes.

#### 3a. Post-GREEN review

1. **Always** — run `ruff check . --select=F401,F811,F841` on all changed files and address any findings
2. **When non-trivial** (any changed function exceeds 20 lines, nesting exceeds 4 levels, or more than one function was modified) — also run `python -m mypy modules/` and address type errors
3. Collect all errors and warnings — these are the refactor candidates for this cycle
...
**A failing test you can't explain is a debugging task, not a restart.** Do not delete code to escape an unexplained failure. Reproduce the failure, find the root cause (per RULE-DBG1/DBG2); only once you understand *why* it failed do you decide whether the fix is code or a restart from RED.
...
## NetSentinel scope notes

**Apply strict TDD (this skill) to:** `modules/*.py` — pure Python, no PyQt, easy to test in isolation.

**Apply RULE-T7 (behavioral test after GREEN) to:** `ui/pages/*.py` — PyQt6 widget lifecycle constraints (RULE-WIN3 through WIN6) make strict test-first impractical. Write the behavioral integration test after the page renders cleanly.
