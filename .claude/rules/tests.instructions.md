...
For any function that processes a list (MetricStore queries, scan result
processing, device list operations), add a scaling guard test alongside the
unit tests. The pattern: measure median wall time at two input sizes, assert
the ratio stays below a threshold.
...
Threshold guide: O(n) → ratio ≈ 10×, use `< 15`. O(n log n) → ratio ≈ 13×,
use `< 20`. Set threshold at ~30% above the theoretical baseline to absorb CI
noise. Skip (not fail) when the small run is below 1e-7 s — too fast to measure.

---

## LOC budget tests — enforce RULE-AH1 structurally

Architecture invariants (file size limits, required package structure) belong
in `tests/test_module_loc.py`. Pattern: glob the target directory, compare
line counts against a budget dict, emit a clear error message that says what
split to make. Do NOT trim cosmetically to pass the test — that defeats the
purpose.
...
`pyproject.toml`. Instead apply the ratchet rule whenever you check coverage:
...
3. **Never lower the gate.** A regression means new code was shipped without tests — add them.
