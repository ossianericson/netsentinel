...
## Process Quality — Lessons from 474 Commits

These rules were derived from a full commit-history audit (474 commits, May–June 2026).
Each rule maps to a recurring failure pattern in the history. The root causes were:
fix:feat ratio 1.14:1; 26 CodeQL-fix commits; 3 Snap Layout reverts; onboarding rebuilt 10+ times;
dashboard.py grew to 13,483 lines; 5 orphaned pages; "resolve 100 CodeQL alerts" in one commit.
...
This rule exists because onboarding was rebuilt 10+ times across Sprints H1, H10, H11, I1–I4,
v1.9.93, and UX Sprint 9 — each replacing the previous without ever declaring the previous one
broken against specific criteria.
...
This rule exists because the Snap Layouts feature was attempted 7 times across 3 feat+revert
cycles. Every constraint (`nativeEvent` + `WM_NCLBUTTONDOWN/UP` + `HTMAXBUTTON` + frozen-exe
crash) was discoverable from MSDN before writing code. A spike would have cost 1 commit;
the revert cycle cost 7.
...
This rule exists because "fix: purge all stale nav labels — Log Hub→Network Logger,
Geo Map→Geolocation Map, Threat Intelligence→Threat Intel, Timeline→Network Timeline"
was a single commit fixing 4+ renames that had been done incompletely.
...
This rule exists because the Network Map had 5 post-feat fix commits across 3 days
(node mis-positioning, grey nodes after restart, layout drift, gateway deduplication,
health lines through wrong nodes) — all discovered by manual use, none by the test suite.

### RULE-STAB1 (required): Budget 20% of every sprint for stabilization of the previous sprint

The "feat: Sprint N — ..." → "fix: Sprint N bugs" → "fix: more Sprint N bugs" pattern
repeated for every major feature area in this codebase (Network Map, Service Heartbeat,
Plugin System, Topology). The fix commits were not anomalies — they were predictable.
...
This rule exists because onboarding was shipped in an unfinished state 10+ times, each time
as the active default, causing the previous working implementation to be overwritten before
the replacement was ready.

### RULE-CHAOS1 (required): Run a 30-minute monkey session before every version bump

The 9-hour chaos run that cleared the app for Microsoft Store submission found multiple bugs
(cross-thread safety, focus-steal, WM_CLOSE to Desktop, HubCard blacklist gaps) that the
4,100+ unit tests did not catch. These bugs are only findable by random UI interaction at scale.
...
High-velocity days (14 commits in one day was observed) correlate directly with increased
fix density in the subsequent 2–3 days. When 8 commits are reached in a calendar day,
...
This rule exists because `dashboard.py` grew to 13,483 lines — requiring 5 dedicated
decomposition sprints (Sprint 15–20) of pure architectural debt repayment. The split should
have been enforced at 600 lines, not discovered at 13,000.
