...
## 2a. Platform setup {#platform-setup}

### Windows

Install [Npcap](https://npcap.com) for packet-capture features (STP, storm detection, ARP monitor, DHCP detector). Run as Administrator for those pages.

If the installer is blocked on first run: right-click the downloaded file → **Properties** → check **Unblock** → **OK**, then run it. This does not apply to winget installs.
...
First launch requires right-click → **Open** to bypass Gatekeeper. Run with `sudo python app.py` to enable packet capture features. On Apple Silicon, ensure you are using a native arm64 Python build — x86_64 via Rosetta may have issues with Scapy and libpcap.
...
If the app fails to launch with a Qt platform plugin error:
...
**Step 2 — Instantiate the page in `_init_pages()`**

Open `ui/dashboard.py` and add the instantiation to `_init_pages()`:
...
self._nav_add_rail_item("◈ My Page", self._my_page)
```

The label string is the flyout item label — it must start with a geometric Unicode prefix character (see RULE-I4). Do **not** call `_nav_add_page()` — that method is legacy dead code.

For pages that require admin privileges, add the keyword argument:
```python
self._nav_add_rail_item("◈ My Page", self._my_page, admin_required=True)
```

For Security Audit section pages, also pass `audit_item=True`:
```python
self._nav_add_rail_item("◈ My Page", self._my_page, admin_required=True, audit_item=True)
```
