...
| # | Feature Name | Priority | Benchmark Source(s) | Why It Matters for NetSentinel | Complexity |
|---|---|---|---|---|---|
| V1 | **Traffic overlay on topology map** — colour-code or animate topology edges by current bandwidth utilization (a "Weathermap" for the home lab) | **High** | LibreNMS Weathermap plugin, ntopng | The Cytoscape.js topology page (Sprint 6) shows device relationships but not traffic load. Seeing which links are saturated at a glance is the #1 use case for NOC-style dashboards. Would require per-interface bandwidth feeds from the existing bandwidth monitor. | Medium |
...
| V5 | **Scan comparison / diff view** — visual side-by-side or diff overlay between two scan results (new ports opened, devices added/removed, services changed) | **Medium** | Nmap ndiff, Netdisco history | `config_baseline.py` does device-level diff but not port/service-level diff. "Last Tuesday port 4444 appeared on your NAS" is a security finding that requires scan comparison. | Medium |
...
| Traffic Analysis | ◆◆◇◇◇ Partial | Application-layer flow breakdown (V2, A2) |
