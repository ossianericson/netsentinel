...
**Sprint 16 queue:** Wire `_DiagExtraTabsMixin` inheritance in `tabs_diag.py` (MTR/tools methods duplicated between `tabs_diag.py` and `tabs_diag_extra.py` — Sprint 13 partial never wired); tighten KNOWN_LARGE_UI_FILES budgets for Sprint 15 new files (home_data_mixin.py 907 lines, tabs_logger.py 772 lines, home_widgets.py 1,316 lines); any new findings from runtime audit.
