...
**Sprint 19 queue:** Further dashboard.py reduction toward 3,000 line target (currently 4,092); largest remaining blocks: nav helper methods (~400 lines in `_rebuild_nav_for_mode`, `_nav_add_section`, etc.), verdict/badge/pill methods (`_update_overall_verdict` 91 lines, `_push_monitor_pills` 99 lines, `_refresh_section_badges` 87 lines), launch/module wiring methods (`_launch_modules_impl` 88 lines); target for sprint 19: ≤3,500 lines.
