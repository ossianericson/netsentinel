...
class MeshClientList(list):
    ...
    def __eq__(self, other: object) -> bool:
        """Two of these are the same result only if they saw the same nodes.

        The whole point of the class is that a partial list is not a complete one, so
        ``failed_nodes`` has to count (CodeQL py/missing-equals). Against a plain
        ``list`` it defers, and ``list.__eq__`` compares contents exactly as before —
        no existing caller changes.
        """
        if isinstance(other, MeshClientList):
            return list(self) == list(other) and self.failed_nodes == other.failed_nodes
