...
def generate_card_html(card_data: dict) -> str:
    ...
    _gc = {
        "A": EXPORT_GREEN_FG, "B": EXPORT_GREEN_FG,
        "C": EXPORT_AMBER_FG, "D": EXPORT_RED_FG, "F": EXPORT_RED_FG,
    }.get(grade, EXPORT_ACCENT_FG)
...
.card-meta {{ color:{EXPORT_META}; font-size:.8rem; margin-top:4px; }}
.card-title {{ font-size:1rem; font-weight:bold; color:{EXPORT_HEADING_FG}; }}
.card-isp {{ font-size:.85rem; color:{EXPORT_META}; margin-top:2px; }}
hr {{ border:none; border-top:1px solid {EXPORT_BORDER}; margin:18px 0 12px; clear:both; }}
ul {{ padding-left:18px; margin:0; }}
li {{ font-size:.88rem; color:{EXPORT_TEXT}; line-height:1.6; }}
.attr {{ font-size:.72rem; color:{EXPORT_META}; margin-top:16px; border-top:1px solid {EXPORT_BORDER}; padding-top:10px; }}
</style>
</head>
<body>
<div class="card-outer">
  <div class="grade-circle">{grade}</div>
  <div class="card-title">NetSentinel — Network Health Card</div>
  <div class="card-isp">{isp}</div>
  <div class="card-meta">Score {score:.0f}/100 &nbsp;·&nbsp; {device_count} device(s) &nbsp;·&nbsp; {generated_at}</div>
  <hr>
  <ul>{finding_rows}</ul>
  <div class="attr">Generated by <a href="https://github.com/ossianericson/netsentinel">NetSentinel</a> — free, open-source network troubleshooting</div>
</div>
</body>
</html>"""
