...
_WHATS_NEW_VERSION = "2.4.0"
_WHATS_NEW_ENTRIES = [
    ("NetSentinel now tells you when NetSentinel itself is the problem", "A new strip on the Home page, and a dot on the tray icon, show when something inside the app has stopped working — a monitor whose checks keep failing, a listener that could not open its port, an alert channel that stopped delivering, or a password that could not be saved. Each one says what stopped, why, what to do about it, and gives you a button to the page that fixes it. It stays up while the problem lasts and clears itself the moment the next check succeeds."),
    ("Failures no longer look like success", "A failed diagnosis used to turn its status dot green and keep the previous run's verdict. A speed test that failed while you were looking at another page recorded nothing at all. A watch that hit an error went back to looking like it had never been run. Every check that can report progress can now also report failure, so what you see is what actually happened."),
    ("Fixed successful exports reporting themselves as failures", "Exporting all data, copying a summary, copying an ISP complaint and saving the network map each ran correctly and then showed you a failure message — the code that was supposed to confirm the success was itself broken, and its error was caught as though the export had failed."),
    ("Error messages now say what to do next", "Across the app, a failure that used to show a raw Windows error now says what failed, why it most likely failed, and what to try — with the original technical text still available on hover or in the log. On non-English systems those raw errors arrived in the system language, which helped nobody."),
    ("Fixed the window freezing during IoT anomaly monitoring", "A few minutes after starting anomaly monitoring, the window could turn white and stop responding. A device answering normal traffic could raise thousands of alerts a minute, and the app tried to draw all of them at once. Alerts are now raised once per device and port, and drawn in batches."),
    ("Fixed controls that cut off their own contents", "Number boxes were too narrow for the values they hold — the port-scan rate showed \"500 p\" instead of \"500 pps\" — and error pop-ups cut off exactly the part telling you what to do next."),
]
