...
_WHATS_NEW_VERSION = "2.3.0"
_WHATS_NEW_ENTRIES = [
    ("NetSentinel now tells you when it closed unexpectedly", "If the app was closed by Windows — killed for memory, or stopped by a fault that produces no error message — it previously left no record at all, so there was nothing to report and nothing to look at. NetSentinel now keeps a note of how each session ended and which page you were on, and shows a message on the Home page the next time you open it."),
    ("You can now export a diagnostic report to attach to a bug report", "Found under Send feedback, it gathers what is actually useful for diagnosing a problem — how recent sessions ended, the tail of each log, and what kind of machine you are on. Your IP addresses, MAC addresses, WiFi network names, device names and Windows username are removed before the file is written. Nothing is ever sent anywhere; you choose whether to share the file."),
    ("The command-line and background-service builds no longer crash on accented text", "Both shipped with defects the main app had already had fixed for it: they could stop working when a device or adapter name contained an accented character, and neither kept any crash record at all. The background service was the worse case, because nobody is watching it when it stops."),
    ("More features now give correct answers on non-English Windows", "Rogue IPv6 router detection always reported \"clean\" regardless of what it found. Connectivity tests reported hosts as unreachable when they had in fact answered. WiFi channel readings were wrong for most access points, on every language including English. All three are fixed."),
    ("Fixed three security-scan results that were empty, invented, or truncated", "A credentialed scan's Windows service list always came back empty. The shared-folder scan could list user accounts that do not exist, taken from wording in the output rather than real account names. Device names using a non-Latin alphabet were cut short instead of shown."),
    ("Fixed PDF export reporting success while producing an error page", "If your Windows username contained a \"#\", the PDF export handed the browser an address it could not read, and saved the browser's own \"file not found\" page as your report — while telling you the export had worked."),
]
