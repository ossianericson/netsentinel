...
_WHATS_NEW_VERSION = "2.2.6"
_WHATS_NEW_ENTRIES = [
    ("Acknowledging one device's alert no longer silences another", "Device alerts were identified by IP address rather than by the device's MAC. Two devices routinely share an address — a DHCP lease gets reused, or a phone rejoins with a new privacy MAC — and on 7 of the 20 addresses on our reference network that was the case. When it was, acknowledging one device's \"has left your network\" alert also muted the other one, and the two shared a cooldown so the second alert never appeared at all. Alerts are now identified by MAC, which is unique to the device, and alert history shows the device's real name instead of a bare address."),
    ("Fixed devices being identified as the wrong device", "Passive network observations (the announcements devices broadcast about themselves) were matched to a device by IP address, and the fix released in v2.2.4 to match by MAC never actually took effect — the lookup it depended on was never called, so every observation still fell back to the address. A powered-off TV could be relabelled a Router / Gateway and then a Smart Speaker. Checked against live traffic: 8 of 8 observations now carry a correctly resolved MAC, where 6 of 7 previously landed on the wrong device."),
    ("Jitter is now measured for your router", "The \"Jitter High\" rule is meant to watch your local connection quality, but jitter was only ever measured against internet test servers — never your own router — so it had no local data at all. On our reference network the router had 1,067 latency samples and not one of them recorded jitter."),
    ("The RTT Anomaly rule is now on by default", "It watches for a device becoming unusually slow compared with its own normal latency, rather than against a fixed number, so it adapts to what is normal for your network. Measured over 28.8 days it produced 0.15 alerts a day — quieter than alerts that already ship switched on. If you have ever saved your notification settings, your own choices are left exactly as you set them."),
]
