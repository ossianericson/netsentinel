...
_WHATS_NEW_VERSION = "2.2.8"
_WHATS_NEW_ENTRIES = [
    ("The app no longer fails silently when something crashes", "If NetSentinel hit an unexpected error and the crash message contained an accented character — in a device name, a WiFi network name, or a file path — the app could fail to write any crash report at all, and could even freeze or crash again while trying to show the error. Both are fixed: crash reports are now always written, and errors from background tasks no longer risk destabilising the whole app."),
    ("Commands like ping, arp, and netsh no longer choke on accented names", "NetSentinel reads output from several built-in Windows tools to gather network details. If a WiFi network or adapter name contained an accented character, that output could be misread and raise an error, silently breaking the feature that needed it. This is now decoded correctly."),
    ("Several features now work correctly on non-English Windows installs", "A handful of features were quietly built around English-only wording: connectivity checks could report a fully working connection as 100% packet loss, and gateway, DHCP, WiFi, SMB, and IPv6 details could fail to load — all without any visible error. These now read the underlying data directly instead of matching English text, so they work the same regardless of your Windows display language."),
    ("Fixed missing or mistimed events in the Network Logger and Device Changes timeline", "Some outage events weren't showing up in the Network Logger at all, and the Device Changes timeline could show events at the wrong time of day. Both were timestamp-handling bugs, now fixed."),
]
