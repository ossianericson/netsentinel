...
_WHATS_NEW_VERSION = "2.2.7"
_WHATS_NEW_ENTRIES = [
    ("Devices now show the right maker", "NetSentinel keeps a small hand-written table of known hardware, and it was being trusted ahead of the official IEEE registry that decides which manufacturer owns which address range. 58 of its entries disagreed with that registry — so a Raspberry Pi on our own network showed up as a Microsoft games console, because its address range had been filed under Xbox. Those 58 entries are gone, and the official registry now has the final say on who made a device. Devices using a randomised (privacy) address, which by design reveals no maker at all, can now take a hint from their name instead of showing a blank."),
    ("Device names are no longer matched inside unrelated words", "A device was classified by looking for clues anywhere in its name, including in the middle of ordinary words. An iPad called \"iPad-2\" was labelled a Domain Controller, because \"ip-ad-2\" contains \"ad-\". A PC called \"Jonas-PC\" became a NAS server, \"Camilla-iPhone\" became an IP camera, and a printer became a Smart TV. Name clues now have to match a whole word, and the fix is guarded by a list of around 150 ordinary first names that must never be mistaken for network equipment."),
    ("What a device can do is now separate from what it is", "When a device announces that it supports AirPlay or Chromecast, that tells you it can play media — not that it is a television. NetSentinel was treating those announcements as proof of identity, and trusting them above everything else, so a speaker or a phone could be relabelled a Smart TV; a device supporting two media standards would flip between identities indefinitely. On our reference network this was behind 69% of all recorded device-type changes, and an iPhone was relabelled a Smart TV twice in a single day. These announcements are now listed under \"Can do\" in the device details drawer, and the device keeps its real identity."),
    ("Devices already in your list get corrected too", "Every fix above only changes how devices are identified from now on, which would leave anything already in your inventory stuck with the old, wrong label until something happened to overwrite it. NetSentinel now makes one pass over your existing devices after upgrading and re-checks them. It only ever improves a label — it will not blank one out, and it never touches a name or type you set yourself. On our reference network it corrected 8 makers and 15 device types."),
    ("Readable tooltips in the Arctic Clean theme", "Tooltips on the Devices page were rendering pale text on a pale background in the light theme, making them effectively invisible. They now carry their own background with them, so they stay legible whichever theme you use."),
]
