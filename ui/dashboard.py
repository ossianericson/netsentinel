...
class Dashboard(QMainWindow):
    ...
    def nativeEvent(self, event_type, message):
        """Return HTMAXBUTTON when hovering the maximize button → Windows Snap flyout."""
        import sys
        if sys.platform == "win32" and event_type == b"windows_generic_MSG":
            try:
                ...
                msg_ptr = int(message)
                # IsBadReadPtr is safe to call with any address — validates before deref.
                if msg_ptr and not ctypes.windll.kernel32.IsBadReadPtr(
                        msg_ptr, ctypes.sizeof(wt.MSG)):
                    msg = wt.MSG.from_address(msg_ptr)
