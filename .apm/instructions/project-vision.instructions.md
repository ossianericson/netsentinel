---
applyTo: "**"
description: "NetSentinel project vision, strategic goals, implemented features, and core product values."
---
...
| 2026-09-05/06 (v2.3.0 release-readiness) | 12 h 06 min (18:46:29–06:52:18), 12 phases, user-interrupted mid soak-lap-2-moderate | 1 coverage cycle + soak lap 1 (full budget) + mild lap 2, ~24,300 interactions in the phases allowed to complete — clean, **0 crashes, 0 app exceptions**. `netsentinel_crash.log` byte-identical at 6,784,379 bytes (mtime still 2026-07-30); `netsentinel_exceptions.log` mtime 2026-09-05 09:57, predating the 18:46 start entirely. Zero tracebacks, zero `0x8001010d`, zero `STATUS_*`, zero hangs. Both systematic sweeps 68/68 pages, `passed=True`, both ending `real-quit phase PASSED` (1.78 s / 2.28 s exits). **Tested exactly the release HEAD** — `45653e3 release: v2.3.0` committed 18:35 on a clean tree, eleven minutes before launch; second run in a row to get this right. **RSS lead stays closed:** medians statistically identical to 2026-08-11 — mild 526 (vs 557.5), moderate 1,108 (vs 1,077), wild **1,244 (vs 1,239)** — and the lap-over-lap reset criterion holds twice over (mild lap 1 526 → lap 2 536 MB in a fresh process; moderate lap 1 1,108 → lap 2 1,041, i.e. *lower*). The single logged exception is the known-harmless pywinauto `COMError -2147220991`, same one as 2026-08-11. The "restart x1" and `[ERROR] Process died` at 06:52:06 are the user's own Ctrl+C, confirmed by `netsentinel_shutdown.log`: a full `closeEvent: real shutdown path` at 06:52:04 completing in 1,203 ms with 27 workers drained, exit code 0 — the app left through the front door and the harness merely saw its child vanish. |
| 2026-09-20/21 (v2.4.0 release-readiness) | 14 h 54 min (16:56:06–07:50:05), 13 phases, user-interrupted mid soak-lap-2-wild | 1 coverage cycle + soak lap 1 (full budget) + lap 2 (mild/moderate full, wild 1 h 40 m of 4 h 30 m), **34,091 interactions** — the largest run in the project's history, against ~27,839 on 2026-08-11. Clean: **0 crashes, 0 app exceptions**, no `STATUS_*`, no `0x8001010d`, no `Fatal Python` in any phase. Both systematic sweeps **68/68**, `passed=True`. `netsentinel_exceptions.log` byte-identical (58,671 b, mtime 2026-09-05, predating the run). **Tested exactly the release HEAD** — `fa13cea release: v2.4.0` committed 16:54:21 on a clean tree, 105 seconds before launch; third run in a row to get this right. **Read the crash log carefully on this one:** it grew +832 b against the pre-session baseline, but its mtime is **16:46:27 — ten minutes BEFORE the run started**. Four `0x8001010d` entries at `app.py:1609` (`_splash.show()`) came from the agent's own live verification script polling pywinauto `connect()` every second from immediately after `Popen`, racing the UIA warmup eight lines later at 1617 — the RULE-WIN10 mechanism, and its own warning that an SEH traceback names the current frame, not the culprit. The run itself drove UIA for 15 h and added **zero bytes**, which is what proves the warmup healthy. Correct baseline for this run is 6,785,211 b / 16:46:27, unchanged end to end. The one logged exception is pywinauto `NoPatternInterfaceError` on a `CheckBox` (no Toggle pattern), DEBUG level, harness-side. The `restart x1` at 07:50:05 is the user's Ctrl+C — `netsentinel_shutdown.log` has `closeEvent: real shutdown path` at 07:50:03 completing in 1,688 ms with 28 workers drained, two seconds before the harness saw the child vanish; all five phase boundaries exited cleanly (1,078–1,890 ms). **RSS lead stays closed:** medians at or below the v2.3.0 baseline on all three levels — mild 523/513 (vs 526), moderate 808/1,031 (vs 1,108), wild 1,144 (vs 1,244). The moderate 808→1,031 lap-over-lap jump is **not** growth: quartiles show both laps ending at the same plateau (Q4 1,112 vs 1,120, 0.7% apart) — lap 1 starts cold at 519 and climbs through Q1–Q2, lap 2 (seed 142, different page order) warms to the plateau inside Q1. The start point differs, the ceiling does not, and lap 1's low median is the anomaly rather than lap 2's. **First chaos validation of the app-health surface, which S10.2 turned on for this release** — and it did more than survive: `speedtest:scheduled` raised *and resolved* mid-run (`condition resolved: speedtest:scheduled after 1 failure(s)`) then raised again, proving the full cycle live. **4 raises + 1 resolve in 15 h, not thousands** — the transitions-only design holding under exactly the load that produced the IoT alert flood this release fixes (RULE-WIN28). |
